...
-(BOOL)closeSerialPortWithPath:(NSString *)path{
    ...
    BOOL closeFlag = [aport close];
    if (closeFlag) {
        [dict_SerialPort removeObjectForKey:path];
    }
    
//    NSLog(@"%d",[aport isOpen]);
    return closeFlag;
}
-(instancetype)init{
    self = [super init];
...
//        dict_config = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
//        NSDictionary *dict_devicepath = [dict_config valueForKey:@"UartDevice"];
        ...
        strCRC = @"";
//        for (NSString *akey in [dict_devicepath allKeys]) {
//            NSDictionary *deviceMessage = [dict_devicepath valueForKey:akey];
//            NSString *path = [deviceMessage valueForKey:@"devicePath"];
//            NSNumber *bandRate = [deviceMessage valueForKey:@"bandRate"];
//            NSString *endSymbol = [deviceMessage valueForKey:@"endSymbol"];
//            ORSSerialPort *aport = [ORSSerialPort serialPortWithPath:path];;
//            aport.baudRate = bandRate;
//            aport.delegate = self;
//            [aport open];
//            if (aport.isOpen) {
//                NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithObjectsAndKeys:aport,@"port",endSymbol,@"es", nil];
//                [dict_SerialPort setObject:dict forKey:aport.name];
//                [[NSNotificationCenter defaultCenter] postNotificationName:@"debugMessage" object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%@: open %@ success",[FileManager TimeStamp],path],@"message", nil]];
//            }else{
//                [[NSNotificationCenter defaultCenter] postNotificationName:@"debugMessage" object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%@: open %@ fail",[FileManager TimeStamp],path],@"message", nil]];
//            }
//        }
    }
    return self;
...
//        NSLog(@"xxxxxxxxxxxxxxxxxxxxxxxx%@xxxxxx%@",data,[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
    }
