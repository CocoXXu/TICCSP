...
- (IBAction)sendMsg:(id)sender {
    ...
    [client writeData:dataCrc withTimeout:1 tag:0];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"debugMessage" object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[FileManager TimeStamp],@"time",[NSString stringWithFormat:@"Send %@:%hu",client.connectedHost,client.connectedPort],@"status",dataCrc,@"message", nil]];
}
...
- (IBAction)ACT_OPEN:(id)sender {
    NSButton *button = (NSButton *)sender;
    if ([button.title isEqualToString:@"打开"]) {
        button.title = @"断开";
        _serverSocket = self.serverSocket = [[GCDAsyncSocket alloc]initWithDelegate:self delegateQueue:dispatch_get_main_queue()];
        NSError *error;
        bool result = [self.serverSocket acceptOnInterface:[_TF_SocketIP stringValue] port:[[_TF_SocketPort stringValue]intValue] error:&error];
        if (result != YES) {
...
{
    // 保存客户端的socket
    [self.dClientSockets setObject:newSocket forKey:[NSString stringWithFormat:@"%@:%hu",newSocket.connectedHost,newSocket.connectedPort]];
    [_POP_Clients removeAllItems];
    [_POP_Clients addItemsWithTitles:self.dClientSockets.allKeys];
    [newSocket readDataWithTimeout:- 1 tag:0];
}
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(nullable NSError *)err{
    for (NSString *asock in self.dClientSockets.allKeys) {
        if ([sock isEqual:[self.dClientSockets valueForKey:asock]]) {
            [_dClientSockets removeObjectForKey:asock];
            break;
        }
    }
    [_POP_Clients removeAllItems];
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"debugMessage" object:nil userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[FileManager TimeStamp],@"time",[NSString stringWithFormat:@"Receive %@:%hu",sock.connectedHost,sock.connectedPort],@"status",data,@"message", nil]];
